...
<div class="formulario">
    <form method="POST" action="" >
     ID: <input type="text" name="txtid"><input type="submit" name="button" value="Search"><br>
...
if (isset($_POST['button'])) {
    ...
    if ($btn=='Listar') {
        ...
        if ($btn=='Modificar'){
            $query ="update clientes set gen_fav_cli='$genero', pais_cli='$pais' where id_cli = '$buscar'";
            $result = pg_query($query);
        }
        if ($btn=='Search'){
            $query = "SELECT * FROM clientes where id_cli = '$buscar'";
            while ($row=pg_fetch_array($result)) {
                echo "<tr><td>".$row['id_cli'].
                "</td><td>".$row['nom_cli']."</td><td>".$row['pri_ape_cli']. "</td><td>".$row['seg_ape_cli'].   
                "</td><td>".$row['cel_cli']."</td><td>".$row['mail_cli']."</td><td>".$row['fec_nac_cli'].
